...
def i_lbound(I,i,n):
    if i<I[0]:
        ...
        return i_min
@jit(nopython=True)
def fast_maxfind(I,P,V,n,m,delta_t,c):
    W = np.zeros((n,m,2))
    for i in range(n):
        ...
        lbound = i_lbound(I,Imin,n)+1
        ubound = i_lbound(I,Imax,n)
        
        for j in range(m):
            vmax = ((Imax-I[ubound])*V[ubound,j]+(I[ubound+1]-Imax)*V[ubound+1,j])/(I[ubound+1]-I[ubound])
            vmax = vmax-tmax*delta_t*P[j]
            vmin = ((Imin-I[lbound-1])*V[lbound-1,j]+(I[lbound]-Imin)*V[lbound,j])/(I[lbound]-I[lbound-1])
            vmin = vmin-tmin*delta_t*P[j]
            if vmax>vmin:
                vbest = vmax
                ubest = tmax
            else:
            ...
            for k in range(lbound,ubound+1):
                test_u = (I[k]-I[i])/delta_t
                test_v = V[k,j]-test_u*delta_t*P[j]
                if test_v>vbest:
                    vbest = test_v
                    ubest = test_u
            W[i,j,0]=vbest
...
def tri_const(a,b,c):
    ...
    return v
class solver:
    def __init__(self,I_grid,P_grid,phi_grid,c,mu,ss,delta_t):
        self.V = phi_grid
    ...
    def max_find(self,i,j):
        ...
        if (vx>vm):
            ...
            return [vm,um]
    def iterate(self):
        W=fast_maxfind(self.I,self.P,self.V,self.n,self.m,self.delta_t,self.c)
...
V = np.zeros((n,m))
tester = first_solver(I,P,V,c,mu,ss,delta_t)
for i in range(1):
    print(i)
...
ax.plot_surface(grid[0],grid[1],tester.controls,color='red')
ax.set_xlabel('I')
ax.set_ylabel('P')
ax.set_zlabel('u')
plt.show()
